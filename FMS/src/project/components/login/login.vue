		...
		<section class="lcsection">
			<ul>
				<li>
					...
					<input type="text" placeholder="请输入用户名" v-model="username" @focus="icondel=true" @blur="icondel=false" class="username" ref="focus" @change="exit">
					<img src="./img/close02.png" class="iconclose" v-if="icondel" @click="username='';$refs.focus.focus()">
				</li>
				<li>
					<img src="./img/Lock.png" class="iconlock">
					<input type="password" placeholder="请输入登录密码" v-model="password" @focus="showcode=true" @blur="showcode=false" ref="changetype">
			...
			login(){
				...
				http.get('login',{username: this.username, password: this.password}).then(res=>{
					if(res.data.status){
						...
						window.sessionStorage.setItem('infotoken',res.data.data.token);
